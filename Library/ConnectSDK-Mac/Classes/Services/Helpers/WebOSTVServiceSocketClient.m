...
//    }
    ...
    return @{
            @"manifestVersion" : @(1),
            @"appId" : @"gzr.lgtv.remotecmd", //[infoDic objectForKey:@"CFBundleIdentifier"],
            @"vendorId" : @"",
            @"localizedAppNames" : @{
                    @"" : @"GZR LGTv Ahihi" //bundleName
            },
...
-(void) helloTv
{
    ...
    NSDictionary *payload = @{
            ...
            @"deviceModel" : @"MacOSX", //ensureString([[UIDevice currentDevice] model]),
...
-(void) registerWithTv
{
    ...
    reg.callbackComplete = ^(NSDictionary* response)
    {
        ...
        {
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hAppDidEnterBackground:) name:NSApplicationDidFinishLaunchingNotification object:nil];
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hAppDidBecomeActive:) name:NSApplicationDidBecomeActiveNotification object:nil];
        }
