...
- (void) launchYouTube:(NSString *)contentId startTime:(float)startTime success:(AppLaunchSuccessBlock)success failure:(FailureBlock)failure
{
    ...
    if (contentId && contentId.length > 0)
    {
        ...
        if ( startTime > 0 ) {
            params = @{ @"contentId" : [NSString stringWithFormat:@"%@&t=%.0f", contentId, startTime]}; //[NSString stringWithFormat:@"%@&pairingCode=%@&t=%.1f", contentId, [[CTGuid randomGuid] stringValue], startTime]
        } else {
...
//
//        NSImage *appIcon = [NSImage imageNamed:imageName];
//        NSString *dataString;
